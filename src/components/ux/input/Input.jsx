...
const Input = (props) => {
  const {
    ...
    placeholder,
    typeheadResults,
    onClick, // add onClick prop to handle the click event
  } = props;
  ...
  return (
    <div className={`relative Finner__container w-full md:w-auto`}>
      <input
        className={`Finner w-full px-8 py-2 capitalize ${
          classes ? classes : ''
        }`}
        type={type || 'text'}
        value={value}
        ...
        onClick={onClick} // ensure the onClick prop is passed down
        style={{ color: '#000' }} // set the text color to dark
      ></input>
      {icon && (
        <FontAwesomeIcon
          icon={icon}
